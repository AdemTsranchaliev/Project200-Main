...
// Models
import { Booking } from '../shared/models/studio/booking.model';
...
export class BookingTableComponent implements OnInit, AfterViewInit {
   ...
   */
  getAllBookings() {
    const bookingsSubscription = this.bookingsService
      .getBookings()
    ...
    setTimeout(() => {
      switch (indexNumber) {
        case 0:
          ...
          break;
        case 1:
          !this.newReservations.paginator
            ? (this.newReservations.paginator = this.paginator2)
            : null;
          break;
        case 2:
          !this.confirmedReservations.paginator
            ? (this.confirmedReservations.paginator = this.paginator3)
            : null;
          break;
        case 3:
          !this.inProgressReservations.paginator
            ? (this.inProgressReservations.paginator = this.paginator4)
            : null;
          break;
        case 4:
          !this.completedReservations.paginator
            ? (this.newReservations.paginator = this.paginator5)
            : null;
          break;
        case 5:
          !this.canceledReservations.paginator
            ? (this.newReservations.paginator = this.paginator6)
            : null;
          break;
      }
    });
